...
Future<void> main() async {
  ...
  Console.info('branch $branch, $commits commits');
  ...
  Console.info('last commit: $subject');
  ...
  Console.info('tracked Dart files: ${dartFiles.lines.length}');
  ...
  if (await which('git') case final git?) {
    ...
    Console.ok('${await git.run(args: ['--version'], quiet: true).text} at $git');
  } else {
  ...
  try {
    ...
    Console.warn('threw, as asked: exit ${e.result.exitCode}');
  }
  ...
  Console.info('did not throw: exit ${probe.exitCode}, ok=${probe.isOk}');
  ...
  Console.info(outcome.fold((e) => 'failed: $e', (text) => text.isEmpty ? 'tree is clean' : 'tree is dirty'));

  Console.rule('Environment');

  // Overrides live in memory; the process environment is never written to.
  Env.set('DEPLOY_ENV', 'staging');
  Console.info('DEPLOY_ENV=${Env.get('DEPLOY_ENV')} (an override, not the real environment)');
  Console.info('CI=${Env.isCI}, PATH has ${Env.require('PATH').split(':').length} entries');
  ...
  Console.ok('the child saw DEPLOY_ENV=$echoed');
  Env.reset();
}
