...
extension on String {
  String cleanWhitespace() => replaceAll(RegExp(r'\s+'), ' ').trim();
  num? extractNumber() {
...
class Markup {
  ...
  /// really is a global.
  Markup $(String selector) => Markup(JQuery.select(_document ?? _elements, selector), false);

  /// Returns all matching elements as a list of scoped [Markup] cursors.
  List<Markup> $$(String selector) => $(selector)._elements.map((element) => Markup([element], false)).toList();
  ...
  List<R> all<R>(String selector, R Function(Markup row) build) =>
      $(selector)._elements.map((element) => build(Markup([element], false))).toList();
  ...
  /// a time.
  Map<String, Object?> extract(Map<String, Object?> schema) => Field._readAll(_root, schema);
  ...
  Element get _root => _document?.documentElement ?? _document?.body ?? _elements.firstOrNull ?? Element.tag('html');

  /// The elements satisfying [test].
  Markup where(bool Function(Element element) test) => Markup(_elements.where(test).toList(), _isXPath);

  /// The elements that themselves match [selector].
  Markup matching(String selector) => where((element) => _matches(element, selector));

  /// The direct children of the current set, optionally matching [selector].
  Markup children([String? selector]) =>
      _collect((element) => element.children.where((child) => selector == null || _matches(child, selector)));
  ...
  Markup siblings([String? selector]) => _collect((element) {
    final parent = element.parent;
    if (parent == null) return const [];
    return parent.children.where((sibling) => sibling != element && (selector == null || _matches(sibling, selector)));
  });
  ...
  /// See [readable].
  String get text => [for (final e in _elements) readable(e)].where((s) => s.isNotEmpty).join(' ');
  ...
  static String readable(Element element) =>
      _preformatted(element) ? element.text.trim() : element.text.cleanWhitespace();
  ...
  /// read together.
  List<String> attrs(String name) => _elements.map((element) => element.attributes[name]).whereType<String>().toList();
  ...
  static bool _matches(Element element, String selector) => JQuery.matches(element, selector);
...
sealed class Field<T> {
  ...
  static AttrField attr(String selector, String attribute) => AttrField(selector, attribute);
  ...
  static Field<num?> number(String selector) => TextField(selector).when((text) => text.extractNumber());
  ...
  static AttrsField attrs(String selector, String attribute) => AttrsField(selector, attribute);
  ...
  static ListField list(String selector, Map<String, Object?> schema) => ListField(selector, schema);

  /// An arbitrary read, for anything the other cases do not cover.
  static CallField<R> custom<R>(R Function(Element element) read) => CallField<R>(read);
  ...
  /// ```
  Field<R> map<R>(R Function(T value) convert) => CallField<R>((root) => convert(read(root)));
  ...
  static Map<String, Object?> _readAll(Element? root, Map<String, Object?> schema) {
  ...
  static Field<Object?> of(Object? spec) {
    switch (spec) {
      ...
      case String css:
        ...
        return AttrField(css.substring(0, at).trim(), css.substring(at + 1).trim());
      ...
      case List<Object?> spec when spec.length == 1:
        ...
        return AttrsField(first.substring(0, at).trim(), first.substring(at + 1).trim());
      case List<Object?> spec when spec.length == 2 && spec[0] is String && spec[1] is Map<String, Object?>:
...
final class AttrField extends Field<String?> {
  ...
  String? read(Element root) {
    ...
    return attribute == 'text' ? Markup.readable(target) : target.attributes[attribute];
  }
}
...
final class TextsField extends Field<List<String>> {
  ...
  @override
  List<String> read(Element root) => [for (final el in root.querySelectorAll(selector)) Markup.readable(el)];
}
...
final class AttrsField extends Field<List<String>> {
  ...
  List<String> read(Element root) {
    final elements = selector.isEmpty ? [root] : root.querySelectorAll(selector);
    if (attribute == 'text') {
...
final class ListField extends Field<List<Map<String, Object?>>> {
  ...
  List<Map<String, Object?>> read(Element root) => [
    for (final el in root.querySelectorAll(selector)) Field._readAll(el, schema),
  ];
}
