...
class JQuery {
  ...
  static int _position(Element element, {required bool ofType, bool fromEnd = false}) {
    final parent = element.parent;
    if (parent == null) return 1;
    final tag = element.localName;
    var index = 0;
    final siblings = parent.children;
    for (var i = fromEnd ? siblings.length - 1 : 0; fromEnd ? i >= 0 : i < siblings.length; fromEnd ? i-- : i++) {
  ...
  static List<String> _branches(String selector) => _branchCache.of(selector, () => _splitSelectorList(selector));

  static List<_Step> _steps(String branch) => _stepCache.of(branch, () => _splitSteps(branch));

  static _ParsedCompound _compound(String selector) => _compoundCache.of(selector, () => _parseCompound(selector));
  ...
  static bool _matchesFrom(Element element, List<_Step> steps, int index) {
    ...
    switch (steps[index].combinator) {
      ...
      case '~':
        for (var sib = element.previousElementSibling; sib != null; sib = sib.previousElementSibling) {
  ...
  static List<_Step> _splitSteps(String selector) {
    ...
    while (i < selector.length) {
      ...
      if (char == '"' || char == "'") {
        ...
        var j = i;
        while (j < selector.length && (selector[j] == ' ' || selector[j] == '\t' || selector[j] == '\n')) {
          j++;
        }
  ...
  static List<Element> _evalStep(List<Element> parents, _Step step, {required bool isFirst, required Object? root}) {
  ...
  static String _unsupported(UnimplementedError error, String css) {
    ...
    return token == null ? 'this selector is not supported: $css' : "'$token' is not a selector this supports";
  }

  static bool _matchesBase(Element el, String baseCss) => (_ambient ?? _MatchCache()).matches(el, baseCss);
  ...
  static void _applyPseudo(
    ...
    switch (name) {
      ...
      case 'nth-last-child':
        final step = _Nth.parse(arg);
        elementFilters.add((el) => step.holds(_position(el, ofType: false, fromEnd: true)));
        break;
      case 'nth-of-type':
        final step = _Nth.parse(arg);
      ...
      case 'nth-last-of-type':
        final step = _Nth.parse(arg);
        elementFilters.add((el) => step.holds(_position(el, ofType: true, fromEnd: true)));
        break;
      case 'first-of-type':
        elementFilters.add((el) => _position(el, ofType: true) == 1);
        break;
      case 'last-of-type':
        elementFilters.add((el) => _position(el, ofType: true, fromEnd: true) == 1);
        break;
      case 'only-of-type':
        elementFilters.add((el) => _position(el, ofType: true) == 1 && _position(el, ofType: true, fromEnd: true) == 1);
        break;
      case 'only-child':
        elementFilters.add(
          (el) => _position(el, ofType: false) == 1 && _position(el, ofType: false, fromEnd: true) == 1,
        );
        break;
      ...
      case 'where':
        ...
        elementFilters.add((el) => branches.any((branch) => JQuery.matches(el, branch)));
        break;
      case 'header':
        elementFilters.add((el) => _headerTag.hasMatch(el.localName ?? ''));
        break;
      case 'input':
        elementFilters.add((el) => _inputTags.contains(el.localName?.toLowerCase()));
        break;
      case 'button':
        elementFilters.add((el) {
      ...
      case 'checkbox':
        elementFilters.add(
          (el) => el.localName?.toLowerCase() == 'input' && el.attributes['type']?.toLowerCase() == 'checkbox',
        );
        break;
      case 'radio':
        elementFilters.add(
          (el) => el.localName?.toLowerCase() == 'input' && el.attributes['type']?.toLowerCase() == 'radio',
        );
        break;
      case 'text':
        elementFilters.add((el) {
      ...
      case 'password':
        elementFilters.add(
          (el) => el.localName?.toLowerCase() == 'input' && el.attributes['type']?.toLowerCase() == 'password',
        );
        break;
      case 'submit':
        elementFilters.add((el) {
      ...
      case 'reset':
        ...
        break;
      case 'empty':
        elementFilters.add((el) => el.nodes.isEmpty || (el.children.isEmpty && el.text.trim().isEmpty));
        break;
      case 'parent':
        elementFilters.add((el) => el.nodes.isNotEmpty && (el.children.isNotEmpty || el.text.isNotEmpty));
        break;
      case 'selected':
        elementFilters.add((el) => el.localName?.toLowerCase() == 'option' && el.attributes.containsKey('selected'));
        break;
      case 'checked':
        elementFilters.add((el) => el.localName?.toLowerCase() == 'input' && el.attributes.containsKey('checked'));
        break;
      case 'disabled':
      ...
      case 'visible':
        ...
        break;
      case 'first':
        positionalFilters.add((list) => list.isNotEmpty ? [list.first] : const []);
        break;
      case 'last':
        positionalFilters.add((list) => list.isNotEmpty ? [list.last] : const []);
        break;
      case 'eq':
        ...
        break;
      case 'even':
        positionalFilters.add((list) => [for (var i = 0; i < list.length; i += 2) list[i]]);
        break;
      case 'odd':
        positionalFilters.add((list) => [for (var i = 1; i < list.length; i += 2) list[i]]);
        break;
      case 'gt':
        final n = int.tryParse(arg ?? '0') ?? 0;
        positionalFilters.add((list) => [for (var i = n + 1; i < list.length; i++) list[i]]);
        break;
      case 'lt':
        final n = int.tryParse(arg ?? '0') ?? 0;
        positionalFilters.add((list) => [for (var i = 0; i < n && i < list.length; i++) list[i]]);
        break;
    }
  }
