...
class ConsoleReader {
  ...
  /// further input; the subscription otherwise ends with the program.
  ...
  /// accepts it.
  ///
  /// An empty answer falls back to [fallback] when given.
  Future<String> ask(
    String question, {
    String? fallback,
  ...
  /// [ArgumentError] when [options] is empty.
  Future<O> pick<O>(
    String question, {
    ...
    while (true) {
      ...
      final choice = int.tryParse((await line())?.trim() ?? '');
