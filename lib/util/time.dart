...
///
/// [formatDuration] renders a **`Duration`**; [timestamp] and [timeAgo] take a
...
/// `formatDuration(-5.s)` is `'-00:05'`. Through 4.0.0 the sign reached the
...
///
/// Uses [date], or the current local time. This is the `DateTime`
/// renderer for a path; [formatDuration] takes a `Duration`.
...
/// `'in the future'`.
String _timeAgo(DateTime past, [DateTime? relativeTo]) {
...
///
/// ISO-8601 is tried first, so anything `DateTime.toString` wrote
...
/// | `20240309_101500`, `20240309` | what [timestamp] writes |
...
final _units = RegExp(r'(\d+(?:\.\d+)?)\s*(ms|s|m|h|d|w)?');
