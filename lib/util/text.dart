...
final _notSlug = RegExp(r'[^\p{L}\p{N}]+', unicode: true);
...
// back positive.
final _digits = RegExp('\\((?:$_magnitude)\\)|-?(?:$_magnitude)');
final _grouping = RegExp(r'[,_ \u00A0\u202F]');
final _wordish = RegExp(r"[\w']+");
...
/// hyphen.
String _slug(String text, {String separator = '-'}) {
  final folded = _foldAccents(text).toLowerCase();
  final hyphenated = folded.replaceAll(_notSlug, '-').replaceAll(_edges, '');
  return separator == '-' ? hyphenated : hyphenated.replaceAll('-', separator);
}

/// What [_foldAccents] replaces, keyed by the accented letter.
///
...
/// most of what [_foldAccents] used to cost.
final Map<int, String> _byRune = () {
...
String _titled(String plain) => plain.length == 1
    ? plain.toUpperCase()
...
///
/// [input] is a `String` or a `List<int>`. A reversible transport encoding, deliberately not filed among the
/// one-way digests in `hash.dart`.
String _toBase64(Object input) => convert.base64.encode(_bytes(input));

/// Reverses [base64], returning the decoded bytes.
///
/// Paired with [_toBase64]; `decode` on its own would say nothing about
/// which encoding.
List<int> _fromBase64(String input) => convert.base64.decode(input);
...
String _clipText(String text, int length, {String ellipsis = '…'}) {
  if (length <= 0) return '';
...
String _cut(String text, int room) {
  var end = room;
...
String _titleCase(String text) => text.replaceAllMapped(
  _wordish,
  (m) =>
...
String _renderTemplate(String template, Map<String, Object?> values) => template
