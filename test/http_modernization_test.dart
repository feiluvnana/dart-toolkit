  ...
  group('v8.0 HTTP Modernization & Ergonomics', () {
    ...
    test(
      ...
      () async {
        ...
        final replies = await urls.parallelMap(Http.get, concurrency: 2);
    ...
    test('One spelling of a request, through Http', () async {
      final reply = await Http.get(at('/json'));
      ...
      final echoReply = await Http.post(
        at('/echo'),
        body: const Body.text('via Http.post'),
      );
      expect(echoReply.text, 'via Http.post');
    });
    ...
    test('Zone-scoped client isolation with Http.using', () async {
      final mock = Fetcher(client: _MockClient());

      await Http.using(mock, () async {
